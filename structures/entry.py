...
class Entry(Hashable):
    ...
    def get_hash(self) -> int:
        ...
        hash_num = self._custom_mmh3_hash(key_value, seed)
        return hash_num

    def _custom_mmh3_hash(self, key, seed):
        ...
        hash_value = seed
        length = len(key)
        rounded_end = (length & ~3)  # Round down to 4 byte block

        # Process the input in 4 byte chunks
        for i in range(0, rounded_end, 4):
            k = (key[i] & 0xff) | ((key[i + 1] & 0xff) << 8) | \
        ...
        if length & 3 == 3:
            k ^= (key[tail_index + 2] & 0xff) << 16
        if length & 3 >= 2:
            k ^= (key[tail_index + 1] & 0xff) << 8
        if length & 3 >= 1:
            k ^= (key[tail_index] & 0xff)
            k *= c1
            k &= 0xFFFFFFFF
            k = (k << r1) | (k >> (32 - r1))
            k &= 0xFFFFFFFF
